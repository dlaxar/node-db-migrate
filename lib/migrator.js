...
Migrator.prototype = {
  ...
  status: function(funcOrOpts, callback) {
    if (dbmUtil.isFunction(funcOrOpts)) {
      funcOrOpts(this.driver, callback);
    } else {
      var self = this;
      ...
      Migration.loadFromFilesystem(self.migrationsDir, function(err, allMigrations) {
        ...
        Migration.loadFromDatabase(self.migrationsDir, self.driver, function(err, completedMigrations) {
          if (err) { callback(err); return; }
          var toRun = filterUp(allMigrations, completedMigrations, '99999999999999', count);

          if(!completedMigrations.length) {
            log.info('No migrations applied by now.');
          } else {
            log.info('These migrations have been applied:');
          }
          ...
          if (toRun.length === 0) {
            ...
            log.info('The following migrations are not applied:');
          }
